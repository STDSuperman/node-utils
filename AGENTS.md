...
pnpm monorepo for CLI tools: repo-do (git repo management) and skill-manager (agent skill symlink management).
...
- **Agent guidance**: `AGENTS.md` is the source of truth for repo instructions; `CLAUDE.md` imports it for Claude Code compatibility
- **Codex skills**: Use `.agents/skills` for repo-scoped Codex skills and `~/.agents/skills` for user-scoped Codex skills; symlink whole skill directories that contain `SKILL.md`
...
- `packages/skill-manager` - agent skills CLI (`skm`)
